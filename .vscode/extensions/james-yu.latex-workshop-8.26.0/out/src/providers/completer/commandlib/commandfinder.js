...
class CommandFinder {
    ...
    getCmdFromNodeArray(file, nodes, cmdNameList = new Set()) {
        let cmds = [];
        nodes.forEach(node => {
    ...
    getCmdFromNode(file, node, cmdNameList = new Set()) {
        const cmds = [];
        if (latex_utensils_1.latexParser.isDefCommand(node)) {
            const name = node.token.slice(1);
            if (!cmdNameList.has(name)) {
                const cmd = new command_1.Suggestion(`\\${name}`, '', { name, args: this.getArgsFromNode(node) }, vscode.CompletionItemKind.Function);
            ...
            if (!cmdNameList.has(node.name)) {
                const cmd = new command_1.Suggestion(`\\${node.name}`, this.whichPackageProvidesCommand(node.name), { name: node.name, args: this.getArgsFromNode(node) }, vscode.CompletionItemKind.Function);
            ...
            if (['newcommand', 'renewcommand', 'providecommand', 'DeclareMathOperator', 'DeclarePairedDelimiter', 'DeclarePairedDelimiterX', 'DeclarePairedDelimiterXPP'].includes(node.name.replace(/\*$/, '')) &&
                ...
                if (!cmdNameList.has(label)) {
                    const cmd = new command_1.Suggestion(`\\${label}`, 'user-defined', { name: label, args }, vscode.CompletionItemKind.Function);
    ...
    getCmdFromContent(file, content) {
        ...
        while (true) {
            ...
            const cmd = new command_1.Suggestion(`\\${result[1]}`, this.whichPackageProvidesCommand(result[1]), { name: result[1], args: this.getArgsFromRegResult(result) }, vscode.CompletionItemKind.Function);
        ...
        while (true) {
            ...
            const cmd = new command_1.Suggestion(`\\${result[1]}`, 'user-defined', { name: result[1], args }, vscode.CompletionItemKind.Function);
            cmd.documentation = '`' + result[1] + '`';
        ...
        if (this.extension.manager.rootFile !== undefined) {
            for (const file of this.extension.manager.getIncludedTeX()) {
                ...
                for (const pkg of cachedPkgs) {
                    const commands = [];
                    this.extension.completer.command.provideCmdInPkg(pkg, commands, new Set());
