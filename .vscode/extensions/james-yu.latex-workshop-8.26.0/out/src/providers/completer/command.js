...
exports.Command = exports.Suggestion = exports.splitSignatureString = void 0;
...
class Suggestion extends vscode.CompletionItem {
...
class Command {
    ...
    provide(languageId, document, position) {
        ...
        const suggestions = [];
        const cmdSignatureList = new Set(); // This holds defined commands signatures
        ...
        const cmdNameList = new Set(suggestions.map(e => e.signature.name));
        this.extension.manager.getIncludedTeX().forEach(tex => {
            var _a;
    ...
    provideCmdInPkg(pkg, suggestions, cmdSignatureList) {
        const configuration = vscode.workspace.getConfiguration('latex-workshop');
        const useOptionalArgsEntries = configuration.get('intellisense.optionalArgsEntries.enabled');
        // Load command in pkg
